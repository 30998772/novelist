"""工具注册中心（对齐 LangGraph-Chatchat tools_factory/tools_registry.py）。

@regist_tool 装饰器在注册 langchain tool 的同时把工具登记进 `_TOOLS_REGISTRY`，
供 graph 装配时按 agent 过滤。所有 skill 转换来的工具都通过它注册。
"""
...
def regist_tool(
    *args: Any,
    ...
    """wrapper of langchain tool decorator, add tool to registry automatically."""
    ...
    def _parse_tool(t: BaseTool):
        ...
        # change default description (one-line)
        if not description:
